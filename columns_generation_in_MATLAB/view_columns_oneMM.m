function view_columns_oneMM(ID,input_dir,output_dir)
...
dwi = MRIread(strcat('/Volumes/newJetStor/newJetStor/paros/paros_WORK/hanwen/ad_decode_test/input/', ID, '/', ID, '_fa_masked.nii.gz'), 0, 1);
volumn = dwi.vol;

% dwi.vol(FSrow+1, FScol+1, FSslice+1)=dwi(FScol, FSrow, FSslice)
if ~isfile(strcat(output_dir, ID, '/columns_1mm/', ID,'_column_lh_dwi.mat'))
    fprintf('Subject %s doesnt have columns files\n', ID);
    return
end
...
columns_num = points_size / 21;
    for i = 1:21
        ...
        interpl = interp3(volumn, rl_cp_dwi(1, index), rl_cp_dwi(2, index), rl_cp_dwi(3, index));
        interpl = interpl(~isnan(interpl));
        rl_values(1,i) = sum(interpl);
    end
...
ylabel('FA');
title(ID);
saveas(gcf, strcat(output_dir, ID, '/columns_1mm/', ID, '_fa.jpg'),'jpg');
writematrix(rl_values, strcat(output_dir, ID, '/columns_1mm/', ID, '_fa.csv'));
% figure;
