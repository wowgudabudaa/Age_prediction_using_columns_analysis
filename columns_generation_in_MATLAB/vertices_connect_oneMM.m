function vertices_connect_oneMM(ID,input_dir,output_dir)
...
%ID = 'S00775';
mkdir(strcat(output_dir, ID, '/columns_1mm'));

lh_white = strcat(output_dir,ID,'/',ID,'/surf/lh.white');
lh_pial = strcat(output_dir,ID,'/',ID,'/surf/lh.pial');
rh_white = strcat(output_dir,ID,'/',ID,'/surf/rh.white');
rh_pial = strcat(output_dir,ID,'/',ID,'/surf/rh.pial');
trans_M_dir = strcat(output_dir,ID,'/DWI2T1_dti.dat');

if ~isfile(lh_white)
    fprintf('Subject %s doesnt have surf files\n', ID);
    return
end
...
% Save paired points
save(strcat(output_dir, ID, '/columns_1mm/', ID, '_pair_lh.mat'), "lh_pair");
save(strcat(output_dir, ID, '/columns_1mm/', ID, '_pair_rh.mat'), "rh_pair");
...
save(strcat(output_dir, ID, '/columns_1mm/',ID, '_column_lh.mat'), "lh_column_points");
save(strcat(output_dir, ID, '/columns_1mm/',ID, '_column_rh.mat'), "rh_column_points");
% xx_column_points = [[RAS_x_1, RAS_y_1, RAS_z_1,..., RAS_x_21, RAS_y_21, RAS_z_21], [...]]

%% Load transformation matrix
if ~isfile(trans_M_dir)
    fid = fopen(strcat(output_dir,'miss_trans_matrix.txt'), 'w+');
    fprintf(fid, 'Subject %s doesnt have transformation matrix\n', ID);
    fclose(fid);
    return
end
...
lh_cp_dwi = reshape(lh_column_points',3,[]);
%lh_column_points(1:2,:)
%lh_cp_dwi(:,1:42)
...
save(strcat(output_dir, ID, '/columns_1mm/', ID, '_column_lh_dwi.mat'), 'lh_cp_dwi');
save(strcat(output_dir, ID, '/columns_1mm/', ID, '_column_rh_dwi.mat'), 'rh_cp_dwi');
writematrix(lh_cp_dwi, strcat(output_dir, ID, '/columns_1mm/', ID, '_column_lh_dwi.csv'));
writematrix(rh_cp_dwi, strcat(output_dir, ID, '/columns_1mm/', ID, '_column_rh_dwi.csv'));
write_surf(strcat(output_dir, ID, '/columns_1mm/', ID, 'lh_t.white'),lh_white_f, lh_white_faces);
write_surf(strcat(output_dir, ID, '/columns_1mm/', ID, 'rh_t.white'),rh_white_f, rh_white_faces);
