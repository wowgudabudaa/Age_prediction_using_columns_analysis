function view_columns_in_rergions_fn(ID,input_dir,output_dir)
...
for k = 1:size(region_list, 2)
    ...
    if ~isfile(strcat(output_dir, ID, '/label_coord/', lh_region_name))
        fprintf('Subject %s doesnt have label files', ID)
        return
    end
    load(strcat(output_dir, ID, '/label_coord/', lh_region_name));
    load(strcat(output_dir, ID, '/label_coord/', rh_region_name));
...
%     rl_values = rl_values ./ (points_size / points_num);
    ...
    saveas(gcf, strcat(output_dir, ID, '/columns/', ID,'_',region_name,'_fa.jpg'),'jpg');
    writematrix(rl_values, strcat(output_dir, ID, '/columns/', ID,'_rl_',region_name,'_fa.csv'));
    writematrix(lh_values, strcat(output_dir, ID, '/columns/', ID,'_lh_',region_name,'_fa.csv'));
    writematrix(rh_values, strcat(output_dir, ID, '/columns/', ID,'_rh_',region_name,'_fa.csv'));
    close(gcf);
end
% figure;
