function get_columns_in_regions_oneMM_DD(ID,input_dir,output_dir)
...
for k = 1:size(region_list, 2)
    ...
    lh_values = zeros(columns_num, 21);
    for i = 1:21
        index = i:21:(i + 21 * (columns_num-1));
        lh_values(:, i) = interp3(volumn, lh_cp_dwi(1, index), lh_cp_dwi(2, index), lh_cp_dwi(3, index));
    end
    % collect values in right hemi
    columns_num = size(rh_cp_dwi, 2) / 21;
    rh_values = zeros(columns_num, 21);
    for i = 1:21
        index = i:21:(i + 21 * (columns_num-1));
        rh_values(:,i) = interp3(volumn, rh_cp_dwi(1, index), rh_cp_dwi(2, index), rh_cp_dwi(3, index));
    end
%     j = 0;
...
%     rl_values = rl_values ./ (points_size / points_num);
    ...
    writematrix(lh_values, strcat(output_dir, ID, '/columns_1mm/', ID,'_lh_',region_name,'_cols_md.csv'));
    writematrix(rh_values, strcat(output_dir, ID, '/columns_1mm/', ID,'_rh_',region_name,'_cols_md.csv'));
end
clearvars;
end