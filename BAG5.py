...
DEFAULT_OUTDIR = "/Users/alex/AlexBadea_MyGrants/BAG_R01_100325/results/results_HABS091525/"
...
def plot_groups_box_violin(values, groups, title, out_png,
                             xlab=None, ylab=None, extra_text=None,
                             show_title=False, top_pad_frac=0.25, text_fs=9):
    ...
    # Pre-compute range for padding
    y_max = float(np.nanmax(vals)) if len(vals) else 1.0
    y_min = float(np.nanmin(vals)) if len(vals) else 0.0
    ...
    need_bracket = False
    y_top_needed = y_max + span * top_pad_frac
    ...
    if len(labels) == 2 and len(data[0]) > 1 and len(data[1]) > 1:
        ...
        y = y_max + h
        x1, x2 = 1, 2
        plt.plot([x1, x1, x2, x2], [y, y + h*0.25, y + h*0.25, y], linewidth=1)
        plt.text((x1 + x2) / 2, y + h*0.30, f"MWU p={u_p:.3g}, t p={t_p:.3g}", ha="center", fontsize=text_fs)
        plt.text((x1 + x2) / 2, y + h*1.10, f"Δμ={delta_mu:.2f}, d={d_val:.2f}, δ={cd_val:.3f}", ha="center", fontsize=text_fs)
        y_top_needed = max(y_top_needed, y + h*1.6)

    # Extra header text (e.g., interaction p-value) placed inside axes top to avoid clipping
    if extra_text is not None:
        ax = plt.gca()
        ax.text(0.5, 0.98, str(extra_text), transform=ax.transAxes, ha='center', va='top', fontsize=text_fs)

    # Ensure enough headroom so labels don't overlap plot border/title
    plt.ylim(y_min - span*0.05, y_top_needed)
    plt.tight_layout()
    plt.savefig(out_png, dpi=200)
    plt.close()
...
# -----------------------
# Model: BAG (and deltaBAG) ~ CDX_Diabetes * APOE4_Genotype
# -----------------------

def fit_and_save_ols_bag_by_diab_geno(df, y_col, geno_col, diab_col, outdir, log_path):
    try:
        import statsmodels.api as sm
        import statsmodels.formula.api as smf
        HAS_SM = True
        ...
        HAS_SM = False
    if not HAS_SM:
        log("[WARN] statsmodels not available; skipping genotype interaction models.", log_path)
        return None
    if (not geno_col) or (geno_col not in df.columns):
        log("[WARN] APOE4_Genotype not found; skipping genotype interaction models.", log_path)
        return None
    if (not diab_col) or (diab_col not in df.columns):
        log(f"[WARN] Diabetes column {diab_col} not found; skipping.", log_path)
        return None
    if y_col not in df.columns:
        log(f"[WARN] Target {y_col} not found; skipping.", log_path)
        return None
    y = pd.to_numeric(df[y_col], errors='coerce')
    g = normalize_apoe_genotype(df[geno_col])
    d = pd.to_numeric(df[diab_col], errors='coerce')
    m = (~y.isna()) & (~g.isna()) & (~d.isna())
    if m.sum() < 12:
        log(f"[INFO] OLS {y_col} ~ {diab_col}*{geno_col}: too few rows (n={int(m.sum())}); skipping.", log_path)
        return None
    dframe = pd.DataFrame({'y': y[m].values, 'GENO': g[m].astype(str).values, 'DIAB': d[m].astype(int).values})
    model = smf.ols('y ~ C(DIAB) * C(GENO)', data=dframe).fit()
    base = f"MODEL_{y_col}_by_{diab_col}_x_{geno_col}"
    summ_path = os.path.join(outdir, f"{base}_summary.txt")
    with open(summ_path, 'w') as f:
        f.write(model.summary().as_text())
    anova = sm.stats.anova_lm(model, typ=2)
    anova_path = os.path.join(outdir, f"{base}_anova.csv")
    anova.to_csv(anova_path)
    coef = model.params.rename("coef").to_frame()
    conf = model.conf_int(); conf.columns = ["ci_lo","ci_hi"]
    pvals = model.pvalues.rename("pval").to_frame()
    coef_tbl = coef.join(conf).join(pvals)
    coef_path = os.path.join(outdir, f"{base}_coeffs.csv")
    coef_tbl.to_csv(coef_path)
    # Partial eta^2 for the interaction term
    p_int = float(anova.loc['C(DIAB):C(GENO)', 'PR(>F)']) if 'C(DIAB):C(GENO)' in anova.index else np.nan
    ss_int = float(anova.loc['C(DIAB):C(GENO)', 'sum_sq']) if 'C(DIAB):C(GENO)' in anova.index else np.nan
    ss_res = float(anova.loc['Residual', 'sum_sq']) if 'Residual' in anova.index else np.nan
    eta_p = (ss_int/(ss_int+ss_res)) if (np.isfinite(ss_int) and np.isfinite(ss_res) and (ss_int+ss_res)>0) else np.nan
    # Optional pickle
    try:
        import pickle
        pkl_path = os.path.join(outdir, f"{base}.pkl")
        with open(pkl_path, 'wb') as f:
            pickle.dump(model, f)
    except Exception as e:
        pkl_path = ""
        log(f"[WARN] Could not pickle model for {y_col}: {e}", log_path)
    log(f"[MODEL] Saved {y_col} ~ {diab_col}*{geno_col}: summary, anova, coeffs{' and pickle' if pkl_path else ''}.", log_path)
    return {
        'target': y_col,
        'diabetes_col': diab_col,
        'genotype_col': geno_col,
        'n': int(len(dframe)),
        'p_interaction_genotype': p_int,
        'partial_eta2_interaction': eta_p,
...
def main():
    ...
    apoe_geno_col = find_col(df, 'APOE4_Positivity')
    if apoe_geno_col:
    ...
    # Genotype × CDX_Diabetes OLS models for BAG/cBAG and deltaBAGs
    model_rows = []
    cdx_diab_col = find_col(df, 'CDX_Diabetes')
    if apoe_geno_col and cdx_diab_col:
        for y_col in PREDICTORS:
            if y_col in df.columns:
                res = fit_and_save_ols_bag_by_diab_geno(df, y_col, apoe_geno_col, cdx_diab_col, args.outdir, log_path)
                if res:
    ...
    if interaction_rows:
        ...
        # --- Add a significance CSV with BH-FDR and stars ---
        def bh_fdr(pvals: pd.Series) -> np.ndarray:
            ...
            ranks = np.arange(1, len(p_clean) + 1)
            q = np.empty_like(p_clean, dtype=float)
            q[order] = p_clean[order] * len(p_clean) / ranks
        ...
        inter_sig = inter_df.copy()
        if 'p_interaction' in inter_sig.columns:
            inter_sig['sig_p_lt_0.05'] = inter_sig['p_interaction'].astype(float) < 0.05
            inter_sig['sig_p_lt_0.01'] = inter_sig['p_interaction'].astype(float) < 0.01
            inter_sig['sig_p_lt_0.001'] = inter_sig['p_interaction'].astype(float) < 0.001
            inter_sig['p_stars'] = inter_sig['p_interaction'].apply(star_code)
            inter_sig['fdr_q'] = bh_fdr(inter_sig['p_interaction'].astype(float))
            inter_sig['sig_fdr_lt_0.05'] = inter_sig['fdr_q'] < 0.05
            inter_sig['sig_fdr_lt_0.10'] = inter_sig['fdr_q'] < 0.10
        inter_sig_csv = os.path.join(args.outdir, "interaction_significance.csv")
        inter_sig.to_csv(inter_sig_csv, index=False)
        print(f"Saved interaction significance table: {inter_sig_csv}")
